...
def shader_oneway_glass(nw: NodeWrangler, color=None, is_window=False, **kwargs):
    transmission = 1
    rgb = nw.new_node(Nodes.RGB)

    rgb.outputs[0].default_value = (uniform(), uniform(), uniform(), 1.0)

    transparent_bsdf = nw.new_node(Nodes.TransparentBSDF)

    shader = nw.new_node(
        Nodes.GlossyBSDF, input_kwargs={"Roughness": uniform(0.25, 1), "Color": rgb}
    )
    shader = nw.new_node(
        Nodes.MixShader,
