...
from . import (
    generate_nature,  # noqa F401 # needed for nature gin configs to load  # noqa F401 # needed for nature gin configs to load  # noqa F401 # needed for nature gin configs to load  # noqa F401 # needed for nature gin configs to load
)
...
def initialize_flying_meshes(
    ...
    mat_paths = load_txt_list(Path(__file__).parent / "flying_mats.txt")
    ...
    # bpy.ops.mesh.primitive_plane_add(size=10000, location=(0, 0, np.random.uniform(-2, -5)))
...
def main(args):
    ...
    init.apply_gin_configs(
        ...
        config_folders=[
            ...
            "infinigen_examples/configs_flying",
        ],
    )
