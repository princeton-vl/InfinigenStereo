...
from infinigen.assets.materials import glass, invisible_to_camera, wood_old
from infinigen.assets.materials.metal import hammered_metal
...
def set_env_texture():
    ...
    # Add necessary nodes
    output_node = nodes.new(type="ShaderNodeOutputWorld")  # Output node
    output_node.location = (400, 0)

    bg_node = nodes.new(type="ShaderNodeBackground")  # Background node
    bg_node.location = (200, 0)

    env_node = nodes.new(type="ShaderNodeTexEnvironment")  # Environment Texture node
    env_node.location = (0, 0)

    # Load the image texture
    image_dir = "/n/fs/scratch/dy2617/landscape"
    imgs = sorted(glob(f"{image_dir}/*.jpg"))
    image_path = np.random.choice(
        imgs
    )  # Update this path to the actual PNG path (use absolute or relative path)
    env_node.image = bpy.data.images.load(image_path)
    ...
    # Set the World to use the new nodes
    bpy.context.scene.world = world


def generate_world_sphere():
    ...
    # Set object visibility for rays
    sphere.visible_camera = True
    sphere.visible_diffuse = False
    sphere.visible_glossy = False
    sphere.visible_transmission = False
    sphere.visible_shadow = False

    # Switch to Cycles render engine for ray visibility settings to take effect
    bpy.context.scene.render.engine = "CYCLES"


def remove_window_glass():
    split_objs = []
    for obj in bpy.data.objects:
        if "WindowFactory" not in obj.name:
            continue
        for slot in obj.material_slots:
            mat = slot.material
            if mat is None:
                continue
            if "shader_glass" in mat.name:
                logging.info(f"Splitting materials on {obj}")
                obj.select_set(True)
...
def compose_indoors(output_folder: Path, scene_seed: int, **overrides):
    ...
    from infinigen_examples import flying_lighting
    ...
    if not overrides["no_light_aug"]:
        if np.random.uniform() < 0.5:
            background_node.inputs[1].default_value = np.random.uniform(0.18, 0.22)
        else:
            background_node.inputs[1].default_value = np.random.uniform(0.01, 0.05)
    ...
    def solve_large():
        solve_stage_name("on_floor_and_wall", "large", restrict_moves=["addition"])
        solve_stage_name("on_floor_freestanding", "large", restrict_moves=["addition"])

    if not overrides["no_solve"]:
    ...
    def solve_medium():
        solve_stage_name("on_wall", "medium", restrict_moves=["addition"])
        solve_stage_name("on_ceiling", "medium")
        solve_stage_name("side_obj", "medium", restrict_moves=["addition"])
    ...
    def place_floating():
        ...
        facs = [import_item(path) for path in overrides["indoor_facs"]]

        indoor_cutoff = len(facs)

        if overrides["use_nature"]:
            nature_facs = [import_item(path) for path in overrides["nature_facs"]]
            facs += nature_facs * 40
        ...
        placer = FloatingObjectPlacement(
            ...
            indoor_cutoff=indoor_cutoff,
            bbox=solved_bbox,
        )

        floating_objs = placer.place_objs(
        ...
        return floating_objs

    floating_objs = p.run_stage("floating_objs", place_floating, default=None)

    camera_rigs = placement.camera.spawn_camera_rigs(
        camera_rig_config=(
            {"loc": (0, 0, 0), "rot_euler": (0, 0, 0)},
            {
                "loc": (rg(overrides["baseline"]), 0, 0),
                "rot_euler": (0, 0, 0),
            },
        )
    )
    ...
    def pose_cameras():
        ...
        if floating_objs:
            scene_objs = solved_rooms + nonroom_objs + [o for o, _ in floating_objs]
        else:
    ...
    def place_lights():
        total_wattage = np.random.uniform(250, 1250)
        num_lights = np.random.randint(0, 5)

        bbox_min, bbox_max = solved_bbox
        for i in range(num_lights):
            ...
            if np.random.uniform() < 0.5:
                light.data.shadow_soft_size = np.random.uniform(0.5, 3)
            light.data.energy = np.random.uniform(0, total_wattage / num_lights)  # 800
            light.data.color = (
                np.random.random(),
                np.random.random(),
                np.random.random(),
    ...
    def remove_metal(obj):
        ...
        with butil.SelectObjects(obj):
            for index, slot in reversed(list(enumerate(obj.material_slots))):
                ...
                if mat is None:
                    continue
                if "metal" in mat.name or "medal" in mat.name:
                    bpy.context.object.active_material_index = index
                    bpy.ops.object.material_slot_remove()
            if len(obj.material_slots) == 0:
                fac = np.random.choice(mat_facs)
    ...
    if overrides["no_glass"]:
        remove_window_glass()
        for o in bpy.data.objects:
            if "WindowFactory" in o.name:
                continue
            remove_glass(o)
            remove_mirror(o)

    if overrides["no_metal"]:
        for o in bpy.data.objects:
            remove_metal(o)

    if not overrides["no_light_aug"]:
        # p.run_stage("lights_off", turn_off_lights)
        p.run_stage("light_aug", light_aug, use_chance=False)
        p.run_stage(
            "floating_lights",
            place_lights,
            use_chance=False,
    ...
    def material_aug():
        ...
        #     if asset.type == "MESH" and asset.hide_viewport == False and "CeilingLight" not in asset.name and 'WindowFactory' not in asset.name:
        #         remove_mats(asset)
        ...
        #     + list(rooms_split["ceiling"].objects)
        #     + list(rooms_split["floor"].objects)
        # )

        if floating_objs:
            scene_objs = nonroom_objs + [o for o, _ in floating_objs]
        else:
            scene_objs = nonroom_objs

        if overrides["all_metal"]:
            for asset in scene_objs:
                if (
                    asset is not None
                    and asset.type == "MESH"
                    and not asset.hide_viewport
                    and "CeilingLight" not in asset.name
                ):
                    remove_mats(asset)
                    if np.random.uniform() < 0.5:
                        hammered_metal.apply(asset)
                    else:
                        glass.apply(asset)

        if overrides["all_wood"]:
            for asset in scene_objs:
                if (
                    asset is not None
                    and asset.type == "MESH"
                    ...
                    remove_mats(asset)
                    wood_old.apply(asset)

        elif overrides["no_tex"]:
            for asset in scene_objs:
                if (
                    asset is not None
                    and asset.type == "MESH"
            ...
            for asset in scene_objs:
                if (
                    ...
                    fac = np.random.choice(mat_facs)
                    logging.info(f"randomizng {asset.name} mat")
                    remove_mats(asset)
                    fac = np.random.choice(mat_facs)
                    if fac != "None":
                        fac.apply(asset)

        # if floating_objs is not None:
        #     if overrides["repeat_mat"] and np.random.uniform() < 0.5:
        #         fac = np.random.choice(mat_facs)
        #         for o, is_indoors in floating_objs:
        #             if is_indoors:
        #                 remove_mats(o)
        #                 fac.apply(o)

        if overrides["no_mats"]:
            for o, _ in floating_objs:
                remove_mats(o)

        # if np.random.uniform() < 0.5:
        #     for obj in rooms_split["wall"].objects:
        #         remove_mats(obj)

    p.run_stage("mat_aug", material_aug, use_chance=False)

    def disable_object_reflections():
        for obj in bpy.data.objects:
            if obj.type == "MESH":
                obj.visible_glossy = False

    # disable_object_reflections()

    def delete_room_aug():
        if np.random.uniform() < 0.5:
            for obj in rooms_split["exterior"].objects:
                butil.delete(obj)
            if np.random.uniform() < 0.5:
                for obj in rooms_split["floor"].objects:
                    butil.delete(obj)
            if np.random.uniform() < 0.5:
                for obj in rooms_split["ceiling"].objects:
                    butil.delete(obj)
            if np.random.uniform() < 0.5:
                for obj in rooms_split["wall"].objects:
                    butil.delete(obj)

    def delete_ceiling_aug():
    ...
    if not overrides["no_light_aug"]:
        p.run_stage("delete_ceiling_aug", delete_ceiling_aug, use_chance=False)

    # for obj in bpy.data.objects:
    #     if obj.name.startswith("PlantContainerFactory"):
    #         butil.delete(obj)
...
def main(args):
    ...
    init.apply_gin_configs(
        ...
        config_folders=[
            ...
            "infinigen_examples/configs_floating",
        ],
    )
